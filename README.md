...
You can then use the script ID (from the script URL) as in the section [Link the library](#library-link)
...
### Listing
```js
var listing = new Vault.Listing();
listing.getMatters(); // will return a list of matters object
listing.getMatters(state); // you can specify a state 
```
#### Methods
|Method|Return type|Description|
---|---|---|
|getMatters(state)|Array|List all the matters corresponding to a given [```state```](#state-id)|
...
|[state](#state-id)| `enum` <br>Matter state|
...
|getState()|STATE|retrieve the [`state`](#state-id) of the active matter|
|setStatus(status)|Undefined|Used to load a matter properties with raw data (use at your own risk)|
...
|addCollaborator(email, role)|Collaborator|Add a collaborator with a given [```role```](#role-id) to the active matter|
|removeCollaborator(email)|"removed"|Removed a collaborator and return a confirmation String|
|createUserExport(account, [exportType](#exportType-id), name, options)|Export|Create an export from a search of a user files|
|*Missing method create Export from Query|
...
|[state](#state-id)| `enum` <br>export state|
